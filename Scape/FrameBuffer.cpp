...
Scape::Rendering::FrameBufferObject::FrameBufferObject(glm::ivec2 Resolution, int Format, bool HasDepth, bool generatemip)
...
Scape::Rendering::FrameBufferObject::FrameBufferObject()
	: Resolution(0), FrameBuffer(0), ColorBuffer(0), DepthBuffer(0), GenerateMip(false) {
}

void Scape::Rendering::FrameBufferObject::Bind() {
	glBindFramebuffer(GL_FRAMEBUFFER, FrameBuffer);
	glViewport(0, 0, Resolution.x, Resolution.y);
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
}

void Scape::Rendering::FrameBufferObject::UnBind(Window Window) {
...
Scape::Rendering::MultiPassFrameBufferObject::MultiPassFrameBufferObject() :
	Resolution(0), FrameBuffer(0), DepthBuffer(0), GenerateMip(false), ColorBuffers{} {
}

void Scape::Rendering::MultiPassFrameBufferObject::Bind() {
	glBindFramebuffer(GL_FRAMEBUFFER, FrameBuffer);
	glViewport(0, 0, Resolution.x, Resolution.y);
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
}

void Scape::Rendering::MultiPassFrameBufferObject::UnBind(Window Window) {
	glBindFramebuffer(GL_FRAMEBUFFER, NULL);
...
void Scape::Rendering::MultiPassFrameBufferObject::UnBind()
{
	glBindFramebuffer(GL_FRAMEBUFFER, NULL);
}
...
void Scape::Rendering::PreparePostProcess() {
	float vertices[] = {
...
void Scape::Rendering::DrawPostProcessQuad() {
	glBindVertexArray(PPQuadVAO);
	glDrawArrays(GL_TRIANGLE_STRIP, 0, 4);
	glBindVertexArray(0);
}
void Scape::Rendering::DrawPostProcessCube() {
	glBindVertexArray(PPCubeVAO);
	glDrawArrays(GL_TRIANGLES, 0, 36);
	glBindVertexArray(0);
}

void Scape::Rendering::DrawWaterQuad() {
	glBindVertexArray(WaterVAO);
	glDrawArrays(GL_TRIANGLE_STRIP, 0, 4);
	glBindVertexArray(0);
}

void Scape::Rendering::DrawPostProcessQuadInstanced(unsigned int InstanceCount)
{
	glBindVertexArray(PPQuadVAO);
...
Scape::Rendering::FrameBufferObjectPreviousData::FrameBufferObjectPreviousData(Vector2i Resolution, int Format, bool HasDepth, bool generatemip) :
...
void Scape::Rendering::FrameBufferObjectPreviousData::Bind(bool Swap) {
	if (Swap)
		Buffer = !Buffer;
...
void Scape::Rendering::FrameBufferObjectPreviousData::BindImage(int Target) {
	Buffers[Buffer].BindImage(Target);
}

void Scape::Rendering::FrameBufferObjectPreviousData::BindDepthImage(int Target) {
	Buffers[Buffer].BindDepthImage(Target);
}

void Scape::Rendering::FrameBufferObjectPreviousData::BindImagePrevious(int Target) {
	Buffers[!Buffer].BindImage(Target);
}

void Scape::Rendering::FrameBufferObjectPreviousData::BindDepthImagePrevious(int Target) {
	Buffers[!Buffer].BindDepthImage(Target);
}

void Scape::Rendering::FrameBufferObjectPreviousData::UnBind(Window& Window) {
	Buffers[Buffer].UnBind(Window);
}

void Scape::Rendering::FrameBufferObjectPreviousData::Swap() {
	Buffer = !Buffer;
}


void Scape::Rendering::CubeMultiPassFrameBufferObject::Bind() {
	glViewport(0, 0, Resolution.x, Resolution.y);

	glBindFramebuffer(GL_FRAMEBUFFER, FrameBuffer);
}

void Scape::Rendering::CubeMultiPassFrameBufferObject::UnBind(Window Window) {
	glBindFramebuffer(GL_FRAMEBUFFER, 0);
	glViewport(0, 0, Window.GetResolution().x, Window.GetResolution().y);
...
void Scape::Rendering::CubeFrameBufferObject::Bind() {
	glViewport(0, 0, Resolution.x, Resolution.y);
	glBindFramebuffer(GL_FRAMEBUFFER, FrameBuffer);
}

void Scape::Rendering::CubeFrameBufferObject::UnBind(Window Window) {
	glBindFramebuffer(GL_FRAMEBUFFER, 0);
	glViewport(0, 0, Window.GetResolution().x, Window.GetResolution().y);
}

Scape::Rendering::CubeFrameBufferObject::CubeFrameBufferObject(Vector2i Resolution, int Format, bool HasDepth) {
	this->Resolution = Resolution;
	glGenFramebuffers(1, &FrameBuffer);
	// create depth cubemap texture
...
Scape::Rendering::MultiPassFrameBufferObjectPreviousData::MultiPassFrameBufferObjectPreviousData(glm::ivec2 Resolution, int stages, std::vector<int> Formats, bool HasDepth, bool generatemip) :
...
void Scape::Rendering::MultiPassFrameBufferObjectPreviousData::Bind(bool Swap) {
	if (Swap)
		Buffer = !Buffer;
...
void Scape::Rendering::MultiPassFrameBufferObjectPreviousData::BindImage(int Target, int TargetImage) {
	Buffers[Buffer].BindImage(Target, TargetImage);
}

void Scape::Rendering::MultiPassFrameBufferObjectPreviousData::BindDepthImage(int Target) {
	Buffers[Buffer].BindDepthImage(Target);
}

void Scape::Rendering::MultiPassFrameBufferObjectPreviousData::BindImagePrevious(int Target, int TargetImage) {
	Buffers[!Buffer].BindImage(Target, TargetImage);
}

void Scape::Rendering::MultiPassFrameBufferObjectPreviousData::BindDepthImagePrevious(int Target) {
	Buffers[!Buffer].BindDepthImage(Target);
}

void Scape::Rendering::MultiPassFrameBufferObjectPreviousData::UnBind(Window& Window) {
	Buffers[Buffer].UnBind(Window);
}

void Scape::Rendering::MultiPassFrameBufferObjectPreviousData::UnBind()
{

}

void Scape::Rendering::MultiPassFrameBufferObjectPreviousData::Swap() {
	Buffer = !Buffer;
}

Scape::Rendering::FrameBufferObjectShadow::FrameBufferObjectShadow(glm::ivec2 Resolution)
...
Scape::Rendering::FrameBufferObjectShadow::FrameBufferObjectShadow()
	: Resolution(0), FrameBuffer(0), DepthBuffer(0) {
}

void Scape::Rendering::FrameBufferObjectShadow::Bind()
{
	glViewport(0, 0, Resolution.x, Resolution.y);
	glBindFramebuffer(GL_FRAMEBUFFER, FrameBuffer);
}

void Scape::Rendering::FrameBufferObjectShadow::UnBind()
{
	glBindFramebuffer(GL_FRAMEBUFFER, 0);
}
